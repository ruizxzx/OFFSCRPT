...
export const CreatorDashboardView: React.FC<Props> = ({ articles, userProfile, onNavigate }) => {
  ...
  const [studioSection,setStudioSection]=useState<'analytics'|'commerce'|'seller'|'earnings'|'digital-products'>('analytics');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 space-y-7">
      ...
      <section className="border-2 border-black bg-white p-2 flex flex-wrap gap-2">
        ...
        <button onClick={()=>setStudioSection('commerce')} className={`border-2 border-black px-4 py-2 font-mono text-[10px] font-black uppercase ${studioSection==='commerce'?'bg-[var(--color-primary)]':'bg-white'}`}>MONETIZATION</button><button onClick={()=>setStudioSection('seller')} className={`border-2 border-black px-4 py-2 font-mono text-[10px] font-black uppercase ${studioSection==='seller'?'bg-[var(--color-primary)]':'bg-white'}`}>SELLER ACCOUNT</button><button onClick={()=>setStudioSection('earnings')} className={`border-2 border-black px-4 py-2 font-mono text-[10px] font-black uppercase ${studioSection==='earnings'?'bg-[var(--color-primary)]':'bg-white'}`}>EARNINGS</button>
      ...
      {studioSection === 'commerce' ? <CommerceFoundationPanel userProfile={userProfile}/> : studioSection === 'seller' ? <CreatorSellerOnboardingPanel userProfile={userProfile}/> : studioSection === 'earnings' ? <CreatorEarningsPanel/> : studioSection === 'digital-products' ? <DigitalProductEnginePanel userProfile={userProfile}/> : <>
