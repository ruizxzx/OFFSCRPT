...
async function fsRunQueryAdvanced(token:string, from:string, filters:any[] = [], orderByOrOptions?:string|{limit?:number;orderBy?:Array<{fieldPath:string;direction:'ASCENDING'|'DESCENDING'}>}, direction:'ASCENDING'|'DESCENDING'='DESCENDING', limitCount?:number){
  let options:{limit?:number;orderBy?:Array<{fieldPath:string;direction:'ASCENDING'|'DESCENDING'}>}={};
...
function v96ReviewIsPublic(f:any){return String(f.status||'')==='published' && String(f.moderationStatus||'approved')==='approved';}
...
async function v96VerifyModerationRole(token:string,uid:string,permission='view',email?:string,emailVerified?:boolean){
  ...
  const permissions=Array.isArray(moderator.fields?.permissions)?moderator.fields.permissions.map(String):[];
  const allowed=permissions.includes('*')||permissions.includes(permission)||permissions.includes('moderation:all');
...
function v96AggregateNext(cur:any,delta:any){const count=Math.max(0,Number(cur.reviewCount||0)+Number(delta.reviewCount||0)); const totalBase=Number.isSafeInteger(Number(cur.ratingTotal))?Number(cur.ratingTotal):Math.round(Number(cur.averageRating||0)*Number(cur.reviewCount||0)); const ratingTotal=Math.max(0,totalBase+Number(delta.ratingTotal||0)); return {reviewCount:count,ratingTotal,averageRating:count?Number((ratingTotal/count).toFixed(2)):0,rating1Count:Math.max(0,Number(cur.rating1Count||0)+Number(delta.rating1Count||0)),rating2Count:Math.max(0,Number(cur.rating2Count||0)+Number(delta.rating2Count||0)),rating3Count:Math.max(0,Number(cur.rating3Count||0)+Number(delta.rating3Count||0)),rating4Count:Math.max(0,Number(cur.rating4Count||0)+Number(delta.rating4Count||0)),rating5Count:Math.max(0,Number(cur.rating5Count||0)+Number(delta.rating5Count||0)),verifiedReviewCount:Math.max(0,Number(cur.verifiedReviewCount||0)+Number(delta.verifiedReviewCount||0))};}
...
async function v96Eligibility(token:string,uid:string,productId:string){
  ...
  const product=await v96GetProduct(token,productId); if(String(product.fields.status||'')!=='active' || String(product.fields.moderationStatus||'active')!=='active')return {eligible:false,reason:'PRODUCT_NOT_ELIGIBLE'};
  if(String(product.fields.creatorId||'')===uid)return {eligible:false,reason:'SELF_REVIEW_NOT_ALLOWED'};
  const orders=await fsRunQueryAdvanced(token,'commerceOrders',[fsFilter('customerId','EQUAL',{stringValue:uid})],{limit:100,orderBy:[{fieldPath:'createdAt',direction:'DESCENDING'}]});
...
async function v96CreateReview(token:string,uid:string,b:any){
  const productId=v96Text(b.productId,200); if(!productId)throw v96Error('Product is required.'); const product=await v96GetProduct(token,productId); const pf=v96Fields(product);
  ...
  const eligibility=await v96Eligibility(token,uid,productId); if(!eligibility.eligible)throw v96Error(eligibility.reason==='ALREADY_REVIEWED'?'This purchase already has a review.':eligibility.reason==='PURCHASE_REQUIRED'?'A verified purchase is required to review this product.':'This purchase is not currently eligible for review.');
  ...
  const title=v96Text(b.title,V96_MAX_REVIEW_TITLE); const body=v96Text(b.body,V96_MAX_REVIEW_BODY); if(body.length<1)throw v96Error('Review text is required.');
  const profile=await fsGet(token,`users/${uid}`); const f=profile?.fields||{}; const now=v96Now(); const reviewId=eligibility.reviewId; const idempotency=String(b.requestId||b.idempotencyKey||reviewId); const review={id:reviewId,productId,creatorId:String(pf.creatorId||''),reviewerId:uid,orderId:String(eligibility.orderId||''),orderItemId:String(eligibility.orderItemId||''),rating,title,body,verifiedPurchase:true,status:'pending',moderationStatus:'queued',reportCount:0,reviewerDisplayName:v96Text(f.displayName||f.name||f.username||'OFFSCRPT user',120),reviewerAvatar:v96Text(f.avatarUrl||f.photoURL||'',500),createdAt:now,updatedAt:now,version:1,requestId:idempotency};
  await fsCreate(token,`commerceReviews/${reviewId}`,review); await fsCreate(token,`commerceModerationCases/${v96Id('case',`review:${reviewId}`)}`,{id:v96Id('case',`review:${reviewId}`),targetType:'review',targetId:reviewId,reportIds:[],priority:'normal',status:'queued',createdAt:now,updatedAt:now,source:'review_submission'}).catch(()=>{});
  await fsCreate(token,`commerceModerationActions/${v96Id('action',`review_created:${reviewId}:${idempotency}`)}`,{action:'review_created',targetType:'review',targetId:reviewId,actorId:uid,actorRole:'buyer',previousState:null,newState:'pending',reasonCode:'other',notes:'',requestId:idempotency,timestamp:now});
...
async function v96UpdateReview(token:string,uid:string,b:any){
  const review=await v96GetReview(token,String(b.reviewId||'')); const f=v96Fields(review); if(String(f.reviewerId||'')!==uid)throw v96Error('You do not own this review.',403,'FORBIDDEN'); if(!['pending','published'].includes(String(f.status||'')))throw v96Error('This review cannot be edited in its current state.',409,'CONFLICT'); const title=v96Text(b.title,V96_MAX_REVIEW_TITLE); const body=v96Text(b.body,V96_MAX_REVIEW_BODY); const rating=Number(b.rating); if(!Number.isInteger(rating)||rating<1||rating>5)throw v96Error('Rating must be an integer from 1 to 5.'); if(!body)throw v96Error('Review text is required.'); const now=v96Now(); const wasPublic=v96ReviewIsPublic(f); const next={...f,title,body,rating,status:'pending',moderationStatus:'queued',updatedAt:now,editedAt:now,version:Number(f.version||1)+1}; const delta=v96AggregateDelta(wasPublic?f:null,null); const reqId=String(b.requestId||v96Id('req',`${review.name}:${now}`)); const actionDoc={action:'review_edited',targetType:'review',targetId:v96DocId(review),actorId:uid,actorRole:'buyer',previousState:f.status,newState:'pending',reasonCode:'other',notes:'',requestId:reqId,timestamp:now}; await v96CommitReviewMutation(token,review,next,delta,actionDoc,reqId); return {review:v96CleanReviewPublic({fields:next,name:review.name}),status:'pending'};
}
...
async function v96ListPublicReviews(token:string,productId:string,params:any){
  const product=await v96GetProduct(token,productId); if(String(product.fields.status||'')!=='active' || String(product.fields.visibility||'')!=='public' || String(product.fields.moderationStatus||'active')!=='active')return {reviews:[],nextCursor:null,aggregate:await v96GetAggregate(token,productId)};
  const sort=v96Text(params.sort,'20'); const normalized=V96_PUBLIC_SORTS.has(sort)?sort:'recent'; const limitCount=Math.max(1,Math.min(50,Number(params.limit||20))); const filters=[fsFilter('productId','EQUAL',{stringValue:productId}),fsFilter('status','EQUAL',{stringValue:'published'})]; let order=[{fieldPath:'createdAt',direction:'DESCENDING' as const}]; if(normalized==='highest')order=[{fieldPath:'rating',direction:'DESCENDING' as const},{fieldPath:'createdAt',direction:'DESCENDING' as const}]; if(normalized==='lowest')order=[{fieldPath:'rating',direction:'ASCENDING' as const},{fieldPath:'createdAt',direction:'DESCENDING' as const}]; if(normalized==='verified')order=[{fieldPath:'verifiedPurchase',direction:'DESCENDING' as const},{fieldPath:'createdAt',direction:'DESCENDING' as const}];
  const rows=await fsRunQueryAdvanced(token,'commerceReviews',filters,{limit:Math.min(100,limitCount+1),orderBy:order}); const reviews=rows.slice(0,limitCount).filter(r=>v96ReviewIsPublic(r.fields)).map(v96CleanReviewPublic); const last=rows[limitCount]; const nextCursor=last?encodeCursor(v96DocId(last),String(last.fields?.createdAt||'')):null; return {reviews,nextCursor,aggregate:await v96GetAggregate(token,productId)};
}
async function v96MyReviews(token:string,uid:string,b:any){const limitCount=Math.max(1,Math.min(50,Number(b.limit||20))); const rows=await fsRunQueryAdvanced(token,'commerceReviews',[fsFilter('reviewerId','EQUAL',{stringValue:uid})],{limit:limitCount,orderBy:[{fieldPath:'updatedAt',direction:'DESCENDING'}]}); return {reviews:rows.map(v96CleanReviewPublic)};}
...
async function v96GetTrustSignals(token:string,productId:string,creatorId?:string){
  const product=await v96GetProduct(token,productId); if(String(product.fields.status||'')!=='active'||String(product.fields.visibility||'')!=='public'||String(product.fields.moderationStatus||'active')!=='active')throw v96Error('Product not publicly available.',404,'NOT_FOUND'); const pf=v96Fields(product); const cid=creatorId||String(pf.creatorId||''); const seller=cid?await fsGet(token,`creatorCommerceProfiles/${cid}`):null; const agg=await v96GetAggregate(token,productId); const af=v96Fields(agg); const sellerStatus=String(seller?.fields?.onboardingStatus||'not_started'); const trustStatus=String(seller?.fields?.trustStatus|| (sellerStatus==='active'?'active':'new')); return {productId,creatorId:cid,sellerStatus,trustStatus,productPublished:String(pf.status||'')==='active'&&String(pf.visibility||'')==='public'&&String(pf.moderationStatus||'active')==='active',verifiedReviewCount:Number(af.verifiedReviewCount||0),reviewCount:Number(af.reviewCount||0),averageRating:Number(af.averageRating||0),trustedSeller:trustStatus==='trusted' && Number(af.verifiedReviewCount||0)>=5 && Number(af.averageRating||0)>=4,generatedAt:v96Now()};
}
async function v96GetCreatorTrustSignals(token:string,creatorId:string){
  const products=await fsRunQueryAdvanced(token,'commerceProducts',[fsFilter('creatorId','EQUAL',{stringValue:creatorId}),fsFilter('status','EQUAL',{stringValue:'active'}),fsFilter('visibility','EQUAL',{stringValue:'public'})],{limit:100,orderBy:[{fieldPath:'updatedAt',direction:'DESCENDING'}]});
  let reviewCount=0,verifiedReviewCount=0,totalRating=0; const productSignals=[]; for(const p of products){if(String(p.fields?.moderationStatus||'active')!=='active')continue; const pid=v96DocId(p); const agg=await v96GetAggregate(token,pid); const a=agg.fields||{}; reviewCount+=Number(a.reviewCount||0); verifiedReviewCount+=Number(a.verifiedReviewCount||0); totalRating+=Number(a.averageRating||0)*Number(a.reviewCount||0); productSignals.push({productId:pid,title:String(p.fields?.title||''),reviewCount:Number(a.reviewCount||0),averageRating:Number(a.averageRating||0),verifiedReviewCount:Number(a.verifiedReviewCount||0)});}
  const seller=await fsGet(token,`creatorCommerceProfiles/${creatorId}`); const sellerStatus=String(seller?.fields?.onboardingStatus||'not_started'); const rawTrustStatus=String(seller?.fields?.trustStatus||(sellerStatus==='active'?'active':'new')); const averageRating=reviewCount?Number((totalRating/reviewCount).toFixed(2)):0; const trustedSeller=rawTrustStatus==='trusted'&&verifiedReviewCount>=5&&averageRating>=4; return {creatorId,sellerStatus: sellerStatus==='active'?'active':'not_public',trustStatus:trustedSeller?'trusted':sellerStatus==='active'?'active':'new',productCount:productSignals.length,reviewCount,verifiedReviewCount,averageRating,trustedSeller,products:productSignals,generatedAt:v96Now()};
}

async function v96CreateReport(token:string,uid:string,b:any){
  const targetType=v96TargetType(b.targetType); const targetId=v96Text(b.targetId,200); if(!targetId)throw v96Error('Target is required.'); const reason=v96Text(b.reasonCode,80); if(!V96_REPORT_REASONS.has(reason))throw v96Error('Invalid report reason.'); const description=v96Text(b.description,V96_MAX_REPORT_BODY); const now=v96Now(); let target:any;
  ...
  const day=new Date().toISOString().slice(0,10); const reportId=v96Id('report',`${uid}:${targetType}:${targetId}:${reason}:${day}`); const existing=await fsGet(token,`commerceReports/${reportId}`); if(existing)throw v96Error('You have already submitted this report recently.',409,'REPORT_DUPLICATE');
  ...
  const caseId=v96Id('case',`${targetType}:${targetId}`); const existingCase=await fsGet(token,`commerceModerationCases/${caseId}`); const caseNext={id:caseId,targetType,targetId,reportIds:Array.from(new Set([...(Array.isArray(existingCase?.fields?.reportIds)?existingCase.fields.reportIds:[]),reportId])),priority:existingCase?.fields?.priority||((targetType==='creator'||reason==='fraud_concern')?'high':'normal'),status:(existingCase&&['resolved','dismissed'].includes(String(existingCase.fields?.status||'')))?'queued':(existingCase?.fields?.status||'queued'),assignedTo:existingCase?.fields?.assignedTo,createdAt:existingCase?.fields?.createdAt||now,updatedAt:now,reportCount:Number(existingCase?.fields?.reportCount||0)+1};
  const writes:any[]=[{create:{name:`${firestoreBase()}/commerceReports/${reportId}`,fields:fields(report)}},{create:{name:`${firestoreBase()}/commerceModerationCases/${caseId}`,fields:fields(caseNext)}}]; if(existingCase)writes[1]={update:{name:`${firestoreBase()}/${`commerceModerationCases/${caseId}`}`,fields:fields(caseNext)}};
  if(targetType==='review'){const rf=target.fields; writes.push({update:{name:`${firestoreBase()}/commerceReviews/${targetId}`,fields:fields({...rf,status:'flagged',moderationStatus:'queued',reportCount:Number(rf.reportCount||0)+1,updatedAt:now})}});} await fsCommit(token,writes);
  const reportAction={action:'report_created',targetType,targetId,actorId:uid,actorRole:'buyer',previousState:null,newState:'submitted',reasonCode:reason,notes:description,requestId:reportId,timestamp:now}; await fsCommit(token,[{create:{name:`${firestoreBase()}/commerceModerationActions/${v96Id('action',`${reportId}:created`)}`,fields:fields(reportAction)}},v96AuditWrite(reportAction)]);
...
async function v96ListModerationQueue(token:string,uid:string,email?:string,emailVerified?:boolean,b:any={}){
  await v96VerifyModerationRole(token,uid,'view',email,emailVerified); const limitCount=Math.max(1,Math.min(100,Number(b.limit||25))); const filters:any[]=[]; if(String(b.status||'')){if(!V96_CASE_STATUSES.has(String(b.status)))throw v96Error('Invalid case status.'); filters.push(fsFilter('status','EQUAL',{stringValue:String(b.status)}));} if(String(b.priority||'')){if(!V96_PRIORITIES.has(String(b.priority)))throw v96Error('Invalid priority.'); filters.push(fsFilter('priority','EQUAL',{stringValue:String(b.priority)}));} if(String(b.targetType||'')){filters.push(fsFilter('targetType','EQUAL',{stringValue:String(b.targetType)}));} if(String(b.assignedTo||''))filters.push(fsFilter('assignedTo','EQUAL',{stringValue:String(b.assignedTo)})); const rows=await fsRunQueryAdvanced(token,'commerceModerationCases',filters,{limit:limitCount,orderBy:[{fieldPath:'updatedAt',direction:'DESCENDING'}]}); return {cases:rows.map(r=>({id:v96DocId(r),...r.fields})),nextCursor:null};
}
async function v96GetModerationCase(token:string,uid:string,b:any,email?:string,emailVerified?:boolean){
  await v96VerifyModerationRole(token,uid,'view',email,emailVerified); const caseId=v96Text(b.caseId,200); const row=await fsGet(token,`commerceModerationCases/${caseId}`); if(!row)throw v96Error('Moderation case not found.',404,'NOT_FOUND'); const f=row.fields||{}; const reportIds=Array.isArray(f.reportIds)?f.reportIds.slice(0,50).map(String):[]; const reports=[]; for(const rid of reportIds){const r=await fsGet(token,`commerceReports/${rid}`); if(r)reports.push({id:rid,...r.fields});} const target= f.targetType==='review'?await v96GetReview(token,String(f.targetId)):f.targetType==='product'?await v96GetProduct(token,String(f.targetId)):f.targetType==='creator'?await v96GetCreator(token,String(f.targetId)):await v96GetOrder(token,String(f.targetId)); const actions=await fsRunQueryAdvanced(token,'commerceModerationActions',[fsFilter('targetId','EQUAL',{stringValue:String(f.targetId)})],{limit:100,orderBy:[{fieldPath:'timestamp',direction:'ASCENDING'}]}); return {case:{id:caseId,...f},reports,target:v96PublicTargetSafe(String(f.targetType||''),target),actions:actions.map(r=>({id:v96DocId(r),...r.fields}))};
}
async function v96UpdateReviewModeration(token:string,uid:string,b:any,actor:any){
  const review=await v96GetReview(token,String(b.reviewId||'')); const f=review.fields||{}; const nextStatus=String(b.status||''); if(!V96_REVIEW_STATUSES.has(nextStatus))throw v96Error('Invalid review status.'); if(!v96ValidReviewTransition(String(f.status||''),nextStatus))throw v96Error('Invalid review state transition.',409,'INVALID_STATE_TRANSITION'); const reason=String(b.reasonCode||'other'); if(!V96_MODERATION_REASONS.has(reason))throw v96Error('A valid moderation reason is required.'); const now=v96Now(); const requestId=String(b.requestId||v96Id('req',`${v96DocId(review)}:${nextStatus}:${now}`)); const beforePublic=v96ReviewIsPublic(f); const nextMod=nextStatus==='published'?'approved':nextStatus==='removed'?'removed':(nextStatus==='pending'?'queued':'queued'); const next={...f,status:nextStatus,moderationStatus:nextMod,moderationReason:reason,updatedAt:now,publishedAt:nextStatus==='published'?(f.publishedAt||now):f.publishedAt,removedAt:nextStatus==='removed'?now:f.removedAt,version:Number(f.version||1)+1}; const afterPublic=v96ReviewIsPublic(next); const delta=v96AggregateDelta(beforePublic?f:null,afterPublic?next:null); const actionDoc={action:`review_${nextStatus}`,targetType:'review',targetId:v96DocId(review),actorId:uid,actorRole:actor.role,previousState:f.status,newState:nextStatus,reasonCode:reason,notes:v96Text(b.notes,V96_MAX_MODERATION_NOTE),requestId,timestamp:now}; const result=await v96CommitReviewMutation(token,review,next,delta,actionDoc,requestId); return {review:v96CleanReviewPublic({name:review.name,fields:next}),actionId:v96Id('action',requestId),aggregate:result.aggregate};
}

async function v96ModerationAction(token:string,uid:string,b:any,email?:string,emailVerified?:boolean){
  const targetType=String(b.targetType||''); const action=String(b.action||''); const permission=targetType==='review'?'moderate:reviews':targetType==='product'?'moderate:products':'moderate:sellers'; const actor=await v96VerifyModerationRole(token,uid,permission,email,emailVerified); const requestId=v96Text(b.requestId,200)||v96Id('req',`${targetType}:${String(b.targetId||'')}:${action}:${String(b.currentVersion||'')}`); const reason=String(b.reasonCode||'other'); if(!V96_MODERATION_REASONS.has(reason))throw v96Error('A valid moderation reason is required for this action.');
  if(action==='assign'||action==='unassign'||action==='resolve'||action==='dismiss'||action==='escalate'){
    const caseRow=await fsGet(token,`commerceModerationCases/${String(b.caseId||'')}`); if(!caseRow)throw v96Error('Moderation case not found.',404,'NOT_FOUND'); const cf=caseRow.fields||{}; let nextStatus=String(cf.status||'open'); if(action==='resolve')nextStatus='resolved'; if(action==='dismiss')nextStatus='dismissed'; if(action==='escalate')nextStatus='escalated'; if(action!=='assign'&&action!=='unassign'&&!v96ValidCaseTransition(String(cf.status||''),nextStatus))throw v96Error('Invalid case state transition.',409,'INVALID_STATE_TRANSITION'); const now=v96Now(); const assignedTo=action==='assign'?String(b.assignedTo||''):action==='unassign'?undefined:cf.assignedTo; if(action==='assign')await v96VerifyAssignee(token,assignedTo); const next={...cf,status:action==='assign'||action==='unassign'?String(cf.status||'queued'):nextStatus,assignedTo,updatedAt:now,resolvedAt:['resolved','dismissed'].includes(nextStatus)?now:cf.resolvedAt,resolvedBy:['resolved','dismissed'].includes(nextStatus)?uid:cf.resolvedBy,resolutionCode:action==='resolve'||action==='dismiss'?v96Text(b.resolutionCode||reason,80):cf.resolutionCode,internalNotes:action==='resolve'||action==='dismiss'?v96Text(b.notes,V96_MAX_MODERATION_NOTE):cf.internalNotes}; const actionDoc={action:`case_${action}`,targetType:String(cf.targetType),targetId:String(cf.targetId),caseId:String(b.caseId||''),actorId:uid,actorRole:actor.role,previousState:cf.status,newState:next.status,reasonCode:reason,notes:v96Text(b.notes,V96_MAX_MODERATION_NOTE),requestId,timestamp:now}; await fsCommit(token,[{update:{name:`${firestoreBase()}/commerceModerationCases/${String(b.caseId||'')}`,fields:fields(next)}},{create:{name:`${firestoreBase()}/commerceModerationActions/${v96Id('action',requestId)}`,fields:fields(actionDoc)}},v96AuditWrite(actionDoc),{create:{name:`${firestoreBase()}/commerceTrustSignals/${v96Id('case_action',requestId)}`,fields:fields({targetType:String(cf.targetType),targetId:String(cf.targetId),event:action,caseId:String(b.caseId||''),actorId:uid,createdAt:now,requestId})}}]); if(['resolve','dismiss'].includes(action)){const reportIds=Array.isArray(cf.reportIds)?cf.reportIds.slice(0,50):[]; for(const rid of reportIds){await fsPatch(token,`commerceReports/${String(rid)}`,{status:action==='resolve'?'resolved':'dismissed',updatedAt:now}).catch(()=>{});}} return {case:{id:String(b.caseId||''),...next},actionId:v96Id('action',requestId)};
  }
  ...
  if(targetType==='product'){
    const product=await v96GetProduct(token,String(b.targetId||'')); const pf=product.fields||{}; const nextState=action==='restore'?'active':action==='hide'?'hidden':action==='remove'?'removed':action==='restrict'?'restricted':action==='approve'?'active':String(b.status||''); if(!['active','hidden','removed','restricted'].includes(nextState))throw v96Error('Invalid product moderation action.'); if(actor.role!=='master_admin'&&action==='remove'&&!actor.permissions.includes('moderate:products:destructive'))throw v96Error('Destructive product moderation requires elevated permission.',403); const now=v96Now(); const next={...pf,moderationStatus:nextState==='active'?'active':nextState,trustState:nextState==='active'?'active':'restricted',updatedAt:now}; const audit={action:`product_${action}`,targetType:'product',targetId:v96DocId(product),actorId:uid,actorRole:actor.role,previousState:String(pf.moderationStatus||'active'),newState:nextState,reasonCode:reason,notes:v96Text(b.notes,V96_MAX_MODERATION_NOTE),requestId,timestamp:now}; await fsCommit(token,[{update:{name:`${firestoreBase()}/commerceProducts/${v96DocId(product)}`,fields:fields(next)}},{create:{name:`${firestoreBase()}/commerceModerationActions/${v96Id('action',requestId)}`,fields:fields(audit)}},v96AuditWrite(audit)]); return {product:{id:v96DocId(product),...next},actionId:v96Id('action',requestId)};
  }
  if(targetType==='creator'){
    const creatorId=String(b.targetId||''); if(!creatorId)throw v96Error('Creator is required.'); const seller=await fsGet(token,`creatorCommerceProfiles/${creatorId}`) || {name:`${firestoreBase()}/creatorCommerceProfiles/${creatorId}`,fields:{creatorId}}; const nextTrust=action==='restore'?'active':action==='restrict'?'restricted':action==='suspend'?'suspended':action==='approve'?'active':String(b.trustStatus||''); if(!['active','restricted','suspended'].includes(nextTrust))throw v96Error('Invalid creator trust action.'); const now=v96Now(); const next={...seller.fields,creatorId,trustStatus:nextTrust,updatedAt:now}; const audit={action:`creator_${action}`,targetType:'creator',targetId:creatorId,actorId:uid,actorRole:actor.role,previousState:String(seller.fields?.trustStatus||'active'),newState:nextTrust,reasonCode:reason,notes:v96Text(b.notes,V96_MAX_MODERATION_NOTE),requestId,timestamp:now}; if(seller.updateTime)await fsCommit(token,[{update:{name:`${firestoreBase()}/creatorCommerceProfiles/${creatorId}`,fields:fields(next)}},{create:{name:`${firestoreBase()}/commerceModerationActions/${v96Id('action',requestId)}`,fields:fields(audit)}},v96AuditWrite(audit)]); else await fsCommit(token,[{create:{name:`${firestoreBase()}/creatorCommerceProfiles/${creatorId}`,fields:fields(next)}},{create:{name:`${firestoreBase()}/commerceModerationActions/${v96Id('action',requestId)}`,fields:fields(audit)}},v96AuditWrite(audit)]); return {creator:{id:creatorId,...next},actionId:v96Id('action',requestId)};
  }
...
async function v96TrustHealth(token:string,uid:string,b:any,email?:string,emailVerified?:boolean){
  await v96VerifyModerationRole(token,uid,'view',email,emailVerified); const issues:any[]=[]; const reviews=await fsRunQueryAdvanced(token,'commerceReviews',[],{limit:500,orderBy:[{fieldPath:'updatedAt',direction:'DESCENDING'}]}); const products=await fsRunQueryAdvanced(token,'commerceProducts',[],{limit:500,orderBy:[{fieldPath:'updatedAt',direction:'DESCENDING'}]}); const orders=await fsRunQueryAdvanced(token,'commerceOrders',[],{limit:500,orderBy:[{fieldPath:'createdAt',direction:'DESCENDING'}]}); const reviewKeys=new Set<string>(); const productPublished=new Map<string,any>(); for(const p of products)productPublished.set(v96DocId(p),p.fields);
  for(const r of reviews){const f=r.fields||{}; const key=`${f.reviewerId}:${f.orderItemId||''}`; if(reviewKeys.has(key))issues.push({severity:'critical',type:'duplicate_review',entityId:v96DocId(r)}); reviewKeys.add(key); if(!V96_REVIEW_STATUSES.has(String(f.status||''))||!V96_REVIEW_MODERATION.has(String(f.moderationStatus||'')))issues.push({severity:'critical',type:'invalid_review_state',entityId:v96DocId(r)}); if(f.verifiedPurchase===true){const orderId=String(f.orderId||''); const o=orders.find(x=>v96DocId(x)===orderId); const valid=!!o&&String(o.fields.customerId||'')===String(f.reviewerId||'')&&Array.isArray(o.fields.items)&&o.fields.items.some((it:any)=>String(it.productId||'')===String(f.productId||'')); if(!valid)issues.push({severity:'critical',type:'invalid_verification',entityId:v96DocId(r)});} if(v96ReviewIsPublic(f)&&!productPublished.has(String(f.productId||'')))issues.push({severity:'critical',type:'orphan_published_review',entityId:v96DocId(r)});}
  const aggregates=await fsRunQueryAdvanced(token,'commerceReviewAggregates',[],{limit:500,orderBy:[{fieldPath:'updatedAt',direction:'DESCENDING'}]}); for(const a of aggregates){const pid=v96DocId(a); const expected=v96AggregateFromReviews(reviews.filter(r=>String(r.fields?.productId||'')===pid)); const af=a.fields||{}; if(Number(af.reviewCount||0)!==expected.reviewCount||Number(af.verifiedReviewCount||0)!==expected.verifiedReviewCount||Number(af.rating1Count||0)!==expected.rating1Count||Number(af.rating2Count||0)!==expected.rating2Count||Number(af.rating3Count||0)!==expected.rating3Count||Number(af.rating4Count||0)!==expected.rating4Count||Number(af.rating5Count||0)!==expected.rating5Count)issues.push({severity:'warning',type:'aggregate_mismatch',entityId:pid,expected,actual:af});}
  const reports=await fsRunQueryAdvanced(token,'commerceReports',[],{limit:500,orderBy:[{fieldPath:'updatedAt',direction:'DESCENDING'}]}); const cases=await fsRunQueryAdvanced(token,'commerceModerationCases',[],{limit:500,orderBy:[{fieldPath:'updatedAt',direction:'DESCENDING'}]}); const caseKeys=new Set<string>(); for(const r of reports){const f=r.fields||{}; if(!V96_REPORT_STATUSES.has(String(f.status||'')))issues.push({severity:'critical',type:'invalid_report_state',entityId:v96DocId(r)});} for(const c of cases){const f=c.fields||{}; const k=`${f.targetType}:${f.targetId}`; if(caseKeys.has(k))issues.push({severity:'warning',type:'duplicate_case_target',entityId:v96DocId(c)}); caseKeys.add(k); if(!V96_CASE_STATUSES.has(String(f.status||''))||!V96_PRIORITIES.has(String(f.priority||'')))issues.push({severity:'critical',type:'invalid_case_state',entityId:v96DocId(c)}); for(const rid of Array.isArray(f.reportIds)?f.reportIds.slice(0,50):[]){if(!reports.some(r=>v96DocId(r)===String(rid)))issues.push({severity:'warning',type:'missing_report_reference',entityId:v96DocId(c),reportId:String(rid)});}}
  const actions=await fsRunQueryAdvanced(token,'commerceModerationActions',[],{limit:500,orderBy:[{fieldPath:'timestamp',direction:'DESCENDING'}]}); for(const a of actions){if(!a.fields?.requestId)issues.push({severity:'warning',type:'missing_request_id',entityId:v96DocId(a)});}
  const critical=issues.filter(i=>i.severity==='critical').length; const warnings=issues.filter(i=>i.severity==='warning').length; return {status:critical?'critical':warnings?'warning':'healthy',healthy:critical===0&&warnings===0?1:0,warnings,critical,issues:issues.slice(0,200),checkedAt:v96Now(),recordsScanned:{reviews:reviews.length,reports:reports.length,cases:cases.length,aggregates:aggregates.length,actions:actions.length,products:products.length,orders:orders.length}};
}
async function v96ValidateAggregate(token:string,uid:string,b:any,email?:string,emailVerified?:boolean){await v96VerifyModerationRole(token,uid,'view',email,emailVerified); const productId=v96Text(b.productId,200); if(!productId)throw v96Error('productId is required.'); const expected=await v96RebuildAggregateData(token,productId); const stored=await v96GetAggregate(token,productId); const actual=stored.fields||{}; const mismatches:any={}; for(const k of ['reviewCount','averageRating','rating1Count','rating2Count','rating3Count','rating4Count','rating5Count','verifiedReviewCount'])if(Number(actual[k]||0)!==Number(expected[k]||0))mismatches[k]={expected:expected[k],actual:actual[k]}; return {productId,healthy:Object.keys(mismatches).length===0,expected,actual,mismatches,checkedAt:v96Now()};}
async function v96RebuildAggregate(token:string,uid:string,b:any,email?:string,emailVerified?:boolean){await v96VerifyModerationRole(token,uid,'moderation:aggregate_rebuild',email,emailVerified); const productId=v96Text(b.productId,200); if(!productId)throw v96Error('productId is required.'); const expected=await v96RebuildAggregateData(token,productId); const stored=await v96GetAggregate(token,productId); const before=stored.fields||{}; if(b.repair!==true)return {repaired:false,productId,expected,actual:before}; const requestId=v96Text(b.requestId,200)||v96Id('repair',`${productId}:${Date.now()}`); const now=v96Now(); const aggregateWrite=stored.updateTime?{update:{name:`${firestoreBase()}/commerceReviewAggregates/${productId}`,fields:fields({...expected,productId,updatedAt:now,lastRequestId:requestId})}}:{create:{name:`${firestoreBase()}/commerceReviewAggregates/${productId}`,fields:fields({...expected,productId,updatedAt:now,lastRequestId:requestId})}}; await fsCommit(token,[{create:{name:`${firestoreBase()}/commerceModerationActions/${v96Id('action',requestId)}`,fields:fields({action:'review_aggregate_rebuilt',targetType:'product',targetId:productId,actorId:uid,actorRole:'admin',previousState:JSON.stringify(before),newState:JSON.stringify(expected),reasonCode:'data_quality_repair',notes:'Explicit aggregate repair',requestId,timestamp:now})}},aggregateWrite]); return {repaired:true,productId,expected,actual:expected,requestId};}
